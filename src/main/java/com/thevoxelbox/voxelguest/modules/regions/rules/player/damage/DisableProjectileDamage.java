 ...
 * @author Monofraps
 */
public class DisableProjectileDamage extends GuestRegionRule
{
    @Override
    public boolean check(final Event event, final Region region, final RuleExecutionSettings settings)
    {
        if (event instanceof EntityDamageEvent)
        {
            ...
            {
                return entityDamageEvent.getCause().equals(EntityDamageEvent.DamageCause.PROJECTILE);
            }
        }
    ...
    {
        return new String[]{"DisableProjectileDamage", "DPD"};
    }
}
